...
* when {@link io.vertx.core.net.JksOptions} is {@link io.vertx.core.net.ServerSSLOptions#setTrustOptions set on trust options} then an exact match with the trust store alias is done
...
You can enable SNI on the server by setting {@link io.vertx.core.net.ServerSSLOptions#setSni(boolean)} to `true` and
